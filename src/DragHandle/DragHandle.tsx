...
function DragHandle({
  ...
  function handleTopAndLeftArrowClick(mode: "horizontal" | "vertical", position: number, direction: "left" | "right" | "top" | "bottom") {
    if (SplitUtils.isSectionOpen(position + 1, mode) && SplitUtils.isSectionOpen(position, mode)) {
      SplitUtils.closeSplitter(position, mode, direction);
    } else if (!SplitUtils.isSectionOpen(position + 1, mode) && SplitUtils.isSectionOpen(position, mode)) {
  ...
  function handleBottomAndRightArrowClick(mode: "horizontal" | "vertical", position: number, direction: "left" | "right" | "top" | "bottom") {
    if (SplitUtils.isSectionOpen(position + 1, mode) && SplitUtils.isSectionOpen(position, mode)) {
