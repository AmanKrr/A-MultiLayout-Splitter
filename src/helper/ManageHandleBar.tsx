  ...
  public static showHandleIconOnOpen(
    ...
    if (sections && sections.length > 0) {
      ...
      if (currentTarget && !prevTarget) {
        ...
        (currentHandleBar?.firstChild as HTMLElement)?.classList.remove("disable");
        currentHandleBar?.children[1]?.classList.remove(splitMode === "horizontal" ? "left-margin-fix" : "top-margin-fix");
      } else if (currentTarget && !nextTarget) {
        // remove right icon
        const currentHandleBar = currentTarget.previousElementSibling;
        (currentHandleBar?.lastChild as HTMLElement)?.classList.remove("disable");
        currentHandleBar?.children[1]?.classList.remove(splitMode === "horizontal" ? "right-margin-fix" : "bottom-margin-fix");
      } else {
        // remove current left icon and right icon of previous
        // previous then current
        const previousHandleBar = currentTarget?.nextElementSibling;
        (previousHandleBar?.firstChild as HTMLElement)?.classList.remove("disable");
        previousHandleBar?.children[1]?.classList.remove(splitMode === "horizontal" ? "left-margin-fix" : "top-margin-fix");
        // current
        const currentHandleBar = currentTarget?.previousElementSibling;
        (currentHandleBar?.lastChild as HTMLElement)?.classList.remove("disable");
        currentHandleBar?.children[1]?.classList.remove(splitMode === "horizontal" ? "right-margin-fix" : "bottom-margin-fix");
    ...
    if (sections && sections.length > 0) {
      ...
      if (!SplitUtils.isSectionOpen(sectionNumber, splitMode)) {
        ...
        if (currentTarget && !prevTarget) {
          ...
          currentHandleBar?.children[1]?.classList.add(splitMode === "horizontal" ? "left-margin-fix" : "top-margin-fix");
        } else if (currentTarget && !nextTarget) {
          // remove right icon
          const currentHandleBar = currentTarget.previousElementSibling;
          (currentHandleBar?.lastChild as HTMLElement)?.classList.add("disable");
          currentHandleBar?.children[1]?.classList.add(splitMode === "horizontal" ? "right-margin-fix" : "bottom-margin-fix");
        } else {
          // remove current left icon and right icon of previous
          // previous then current
          const previousHandleBar = currentTarget?.nextElementSibling;
          (previousHandleBar?.firstChild as HTMLElement)?.classList.add("disable");
          previousHandleBar?.children[1]?.classList.add(splitMode === "horizontal" ? "left-margin-fix" : "top-margin-fix");
          // current
          const currentHandleBar = currentTarget?.previousElementSibling;
          (currentHandleBar?.lastChild as HTMLElement)?.classList.add("disable");
          currentHandleBar?.children[1]?.classList.add(splitMode === "horizontal" ? "right-margin-fix" : "bottom-margin-fix");
