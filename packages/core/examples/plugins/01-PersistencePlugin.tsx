...
export default function PersistencePluginExample() {
  return (
    <div style={{ height: '100%', width: '100%' }}>
      ...
      >
        💾 Persistence Plugin Active
        <br />
        <small>Resize and close the browser - layout persists!</small>
      </div>

      <Split id="persistent-example" mode="horizontal" initialSizes={['40%', '60%']} plugins={[myPersistencePlugin]}>
  ...
  debounce: 500,
          ...
          <h3>Storage Key:</h3>
          <p>Data stored at:</p>
          <code style={{ background: '#000', color: '#0f0', padding: '5px 10px', borderRadius: '4px' }}>a-multilayout-splitter:persistent-example</code>
        </div>
      </Split>
    </div>
  );
