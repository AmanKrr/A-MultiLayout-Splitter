...
/**
 * DragHandle
 * Handlebar component with collapse/expand buttons and grip dots.
 */
...
> = ({
  ...
  const handleLeftClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    rightPaneCollapsed ? onExpand?.("right") : onCollapse?.("left");
  };

  const handleRightClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    leftPaneCollapsed ? onExpand?.("left") : onCollapse?.("right");
  };

  const showButtons = !lineBar && !explicitlyDisabled;
  const isHorizontal = mode === "horizontal";

  // Hide buttons that are logically invalid based on current collapse state
      ...
      {showButtons && (
        <>
          <button
            ...
            aria-label={
              rightPaneCollapsed
                ? `Expand ${isHorizontal ? "right" : "bottom"} pane`
                : `Collapse ${isHorizontal ? "left" : "top"} pane`
            }
            title={
              rightPaneCollapsed
                ? `Expand ${isHorizontal ? "right" : "bottom"} pane`
          ...
          <button
            ...
            aria-label={
              leftPaneCollapsed
                ? `Expand ${isHorizontal ? "left" : "top"} pane`
                : `Collapse ${isHorizontal ? "right" : "bottom"} pane`
            }
            title={
              leftPaneCollapsed
                ? `Expand ${isHorizontal ? "left" : "top"} pane`
